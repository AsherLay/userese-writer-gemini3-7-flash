#!/usr/bin/env python3
"""Send traceable batches of user-facing copy to Gemini through OpenRouter."""
...
def request_body(
    run: dict[str, Any], items: list[dict[str, Any]], reasoning_effort: str
) -> dict[str, Any]:
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if retryable and attempt < max_retries:
                ...
                continue
            raise UserError(f"OpenRouter returned HTTP {exc.code}: {detail}") from exc
        except (urllib.error.URLError, socket.timeout, TimeoutError) as exc:
            # A timed-out request may still have been billed. Avoid an ambiguous automatic retry.
            raise UserError(f"OpenRouter request failed without retry: {exc}") from exc
        except json.JSONDecodeError as exc:
            raise UserError("OpenRouter returned invalid JSON") from exc
...
def response_text(response: dict[str, Any]) -> str:
    try:
        ...
        raise UserError(
            "OpenRouter response has no usable choice: "
    ...
    if isinstance(content, list):
        ...
        if combined:
            return combined
    raise UserError("OpenRouter response choice contains no output text")
...
def parse_model_items(response: dict[str, Any]) -> list[dict[str, Any]]:
    try:
        ...
        raise UserError(f"OpenRouter model output is not valid JSON: {exc}") from exc
    items = payload.get("items") if isinstance(payload, dict) else None
    if not isinstance(items, list):
        raise UserError("Gemini output must contain an items array")
    return items
...
def main() -> int:
    ...
    parser.add_argument(
        "--model",
        default=os.getenv(
            "USERESE_WRITER_GEMINI_MODEL",
            os.getenv("WRITER_GEMINI_MODEL", DEFAULT_MODEL),
        ),
    )
    parser.add_argument(
        "--reasoning-effort",
        ...
        default=os.getenv(
            "USERESE_WRITER_GEMINI_REASONING_EFFORT",
            os.getenv(
                "WRITER_GEMINI_REASONING_EFFORT",
                os.getenv("WRITER_GEMINI_THINKING_LEVEL", "low"),
            ),
        ),
    )
    ...
    try:
        ...
        if args.dry_run:
            artifact = {
                ...
                "requests": [
                    {
                        **request_body(run, batch, args.reasoning_effort),
                        "model": args.model,
                    }
        ...
        for index, batch in enumerate(batches, start=1):
            print(f"Calling OpenRouter batch {index}/{len(batches)} ({len(batch)} items)...", file=sys.stderr)
            body = request_body(run, batch, args.reasoning_effort)
